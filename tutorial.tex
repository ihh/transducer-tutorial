...
\end{tabular}
\caption{ \tablabel{tkf91-tkf91} Meaning of states in \figref{tkf91-tkf91}.
...
\end{tabular}
\caption{ \tablabel{mf-substituter} Meaning of states in \figref{mf-substituter}.
...
\end{tabular}
\caption{ \tablabel{mf-tkf91} Meaning of states in \figref{mf-tkf91}.
Since it is an ensemble of two transducers, every state corresponds to a tuple $(i,b)$
where
$i$ is the state of the generator transducer and
...
\end{tabular}
\caption{ \tablabel{tkf91-mf} Meaning of states in \figref{tkf91-mf}.
...
This is correct: the path weight of zero is the probability of transforming MF to LIV by point substitutions alone.  
...
\\
$SI\delta_y$ & The component TKF91 machine emits a character via the Insert state which is read by the $\delta_y$ state of the recognizer; the generator remains in the Start state, not yet having emitted any characters. 
\\
$\imath_x I\delta_y$ & The component  TKF91 machine emits a character via the Insert state which is read by the $\delta_y$ state of the recognizer; the generator remains in an Insert state corresponding to the last character $x$ which it generated. 
\\
$\imath_x M\delta_y$ & The generator emits character $x$ via an Insert state; TKF91 absorbs and emits it via a Match state;  
the recognizer absorbs the character via state $\delta_y$.  
\\
$\imath_x D W_y$ & The generator emits character $x$ via an Insert state, and TKF91 absorbs and deletes it via a Delete state.  
...
\end{tabular}
\caption{ \tablabel{mf-tkf91-liv} Meaning of states in \figref{mf-tkf91-liv}.
...
% crossref
This machine is the ``complete evidence-expanded model'' ($G_0$) described in
