...
\newcommand\titlestring{An automata-theoretic generalization to indels of Felsenstein's phylogenetic pruning algorithm}
...
% crossref
These ideas are described in greater detail in \secref{Transducer}.
...
% crossref
Generators and recognizers are described in more detail in \secref{GenSinglet,ExactMatch}.
...
This is a trivial state machine which emits/generates the sequence MF with probability/weight 1. }

\pngfig{liv-small}{Recognizer for protein sequence LIV.
This is a trivial state machine which absorbs/recognizes the sequence LIV with probability/weight 1, and all other sequences with weight 0.  }
...
% crossref
State types are described in greater detail in \secref{StateTypes}.
...
\widepdffig{legend}{Each transducer state is categorized as one of the above state {\em types} (though a transducer may contain several of each type).  
...
structure.  
Wait states are useful in co-ordinating multiple connected trandsucers, since they require
...
\]
% crossref
These ideas are described in more detail in \secref{Identity}. 
...
The three parameters  relevant for the model are defined as follows:
$a$ is the ``match probability'' (equivalently, $1-a$ is the deletion probability),
$b$ is the probability of an insertion at the start of the sequence or following a match,
and $c$ is the probability of an insertion following a deletion. 

The three I/O functions are defined as follows:
...
\subsubsection{Equilibrium (root) generator} TKF91 is an input/output transducer that
...
The insertion probability is parametrized as the ratio of the birth (insertion) rate $\lambda$ and death (deletion)
rate  $\mu$ so that the distribution of sequences at the root corresponds to the equilibrium
distribution after repeated applications of the TKF91 transducer.  
...
What remains is to detail the various ways of connecting transducers by
 constraining some of their tapes to be the same - namely {\em composition} and {\em forking}.
...
% crossref
Composition is described in greater detail in \secref{Composition}. 
...
As a simple example of transducer composition, we turn to the simple subsituting transducers
...
Constructing the corresponding to $TU$ composite machine 
...
%getrefs ?
\pdffig{mf-substituter}{Transducer $\generate(MF) \cdot \substitute(Q)$.  State types are indicated by the shape, and the I/O functions of the two Insert states are indicated.  }
...
\label{mf-tkf91}
\tallpdffig{mf-tkf91}{Transducer $\generate(MF) \cdot \tkf$, the composition of MF-generator (\figref{moore-mf-generator}) with TKF91 transducer (\figref{tkf91-labeled}).  See Section \ref{mf-tkf91} for the meaning of each state. }

\figref{mf-tkf91} is the composition of MF-generator with TKF91.  
In contrast to $\generate(MF) \cdot \substitute(Q)$, this machine models the generation of 
sequence related to $MF$ by indels and substitutions.  It is similar to a profile HMM
trained on the single sequence $MF$.  
...
A crucial point here is that the {\em composite} machine contains no states of
 type Match or Delete, since it accepts null input (since its ``most upstream'' 
...
\figref{mf-substituter}, but the composition
is reversed.  
The composite machines accept sequence on input, and emit null output.  


\pdffig{substituter-mf}{Transducer $\substitute(Q) \cdot \recognize(MF)$ `recognizes' sequences related to $MF$.  In contrast to the machine in \figref{mf-substituter}, this machine accepts input and emits null output.  Note than a sequence of any length besides 2 will have recognition weight zero. }

\pdffig{substituter-cs}{Transducer $\substitute(Q) \cdot \recognize(CS)$ 'recognizes' sequences related to $CS$.  In contrast to the machine in \figref{cs-substituter}, this machine accepts input and emits null output.  Note than a sequence of any length besides 2 will have recognition weight zero. }
...
\tallpdffig{tkf91-mf}{Transducer $\tkf \cdot \recognize(MF)$ models the evolution
 (according to the TKF91 model) of a sequence resulting in $MF$.  
...
Since it is an ensemble of two transducers, every state of $\tkf \cdot \recognize(MF)$ corresponds to a tuple $(b,d)$
where
$b$ is the state of the TKF91 transducer and
$d$ is the state of the recognizer transducer.
The meaning of the various states in this model are
...
\pdffig{mf-substituter-cs}{Transducer $\generate(MF) \cdot \substitute(Q) \cdot \recognize(CS)$ is a pure Markov chain modeling the transformation of MF to CS via substitutions only. }
...
\widepdffig{mf-tkf91-liv}{Transducer $\generate(MF) \cdot \tkf \cdot \recognize(LIV)$...}
...
Like the machine in \figref{mf-substituter-cs} $\generate(MF) \cdot \tkf \recognize(LIV)$ contains only null states (no match, insert, or delete states), making it a pure Markov chain.  
Markov models are a special case of an input/output machine where the input and output are both null:
...
% crossref
Forking is described in greater detail in \secref{Fork}. 

Forking two transducers is to feed the same input tape into both transducers in parallel.
...
\pdffig{fork-subcs-submf}{The transducer $(\substitute(Q) \cdot \recognize(CS))\fork(\substitute(Q) \cdot \recognize(MF))$  models the distribution of the ancestor common to MF and CS.}

\figref{fork-subcs-submf} shows the fork of \figref{substituter-mf} and \figref{substituter-cs}.  
...
and $\fork$ is the pointwise product.
So, for example,
...
allows computing  the final Felsenstein probability for the two sequence MF and CS related 
...
The previous machine corresponds to the Felsenstein probability computed at each internal
node of a tree.  
...
Composing the transducer
 $(\substitute(Q) \cdot \recognize(CS))\fork(\substitute(Q) \cdot \recognize(MF))$  
with a simple prior over characters $\pi_j$ constructs a pure Markov chain
whose sum-over-paths weight is the final Felsenstein probability (\figref{root-fork-subcs-submf}).  
Furthermore, sampling traceback paths yields through this machine provides an easy
...
\widepdffig{fork-tkf91liv-tkf91mf}{Transducer $(\tkf \cdot \recognize(LIV))\fork(\tkf \cdot \recognize(MF))$ models the ancestor common to LIV and MF, which may differ from each by insertions, deletions, and substitutions.  
This figure was autogenerated using phylocomposer (a DART program) and graphviz.} 

$\tkf \cdot \recognize(LIV))\fork(\tkf \cdot \recognize(MF)$, shown in \figref{fork-tkf91liv-tkf91mf}, is the recognition profile for the common ancestor of LIV and MF.
...
The sum-over-paths weight through this machine corresponds to the final Felsenstein likelihood. 
% crossref
This machine is analagous to the ``full evidence expanded model'' described in
\figref{EvidenceExpandedModel}.  
}

Similar to the case of the  
$(\substitute(Q) \cdot \recognize(CS))\fork(\substitute(Q) \cdot \recognize(MF))$  
transducer, we must compose a prior distribution onto our forked machine in order to compute
the full Felsenstein probability (without having to condition on an input sequence).  
...
$\tkfroot \cdot ((\tkf \cdot \recognize(LIV))\fork(\tkf \cdot \recognize(MF)))$ 
...
\widepdffig{viterbi-fork-tkf91liv-tkf91mf}{Removing the root generator produces a 
...
ancestor relating MF and LIV, created by taking the Viterbi path through the machine in \figref{root-fork-tkf91liv-tkf91mf} and removing the prior (so as to make it a recognizer).}


\subsubsection{Sampling reconstructions of ancestor to MF and LIV}

\widepngfig{forward2-root-fork-tkf91liv-tkf91mf}{
Instead of only saving the highest-weight path through
 $\tkfroot \cdot ((\tkf \cdot \recognize(LIV))\fork(\tkf \cdot \recognize(MF)))$, 
...
It is possible that many paths through the graph have weights similar to the 
Viterbi path.
...
\widepdffig{forward2-fork-tkf91liv-tkf91mf}{Two samples through the 
...
% crossref
The exact transformations required (removing the prior, null states, and adding wait states)
...
Essentially composition replaces matrix multiplication and forking replaces the 
pointwise product.  
...
\widepdffig{fork-tkf91viterbi-tkf91cs}{Transducer $(\tkf \cdot \profile_1)\fork(\tkf \cdot \recognize(CS))$ models the alignment of CS with the most likely recognition profile for MF and LIV, 
all separated by TKF91 transducers.  
This represents the most resource-conservative form of progressive alignment - only the 
...
which summarizes the ancestor of MF and LIV.  
$\profile_2$ is a branched profile whose different paths represent alternate evolutionary
histories relating MF and LIV. }

\figref{fork-tkf91forward2-tkf91cs} shows the fork of the sampled profile $\profile_2$ (summarizing MF and LIV) with the sequence CS.  
...
The resulting transducer is in weak-normal form (it can be converted to a strict-normal form transducer by eliminating null states).
% crossref
In the tutorial section, many examples of simple and complex forks are shown in \secref{Tutorial.Fork}, for instance \figref{fork-subcs-submf} and  \figref{root-fork-tkf91liv-tkf91mf}. 
...
% crossref
\figref{lif-small} shows a small example of an exact-match transducer for sequence LIV.
...
% crossref
\figref{mf-generator} shows a small example of a generating transducer for sequence MF.
