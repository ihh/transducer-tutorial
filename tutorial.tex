...
This is directly analagous to computing a pairwise alignment (e.g. using the Needlman-Wunsch algorithm), and the structure of the Markov model shown in \figref{mf-tkf-liv} suggests the familiar structure of a pairwise dynamic programming matrix.  
...
\pdffig{fork-subcs-submf}{Transducer $(\substitute(Q) \cdot \recognize(CS))\fork(\substitute(Q) \cdot \recognize(MF))$...}
...
(multiplied by geometric prior over sequence length)...
...
\widepdffig{fork-tkf91liv-tkf91mf}{Transducer $(\tkf \cdot \recognize(LIV))\fork(\tkf \cdot \recognize(MF))$ ...
Autogenerated by phylocomposer/graphviz ...}
...
\widepngfig{root-fork-tkf91liv-tkf91mf}{Transducer $\tkfroot \cdot ((\tkf \cdot \recognize(LIV))\fork(\tkf \cdot \recognize(MF)))$...}
