...
% crossref
This machine is equivalent to the model denoted $G_n$ in \secref{EvidenceExpandedModel}
and could also be an example of $H_n$ in \secref{ConstrainedExpanded}.
...
% crossref
This machine is the model denoted $M_n$ in \secref{ConstrainedExpanded}.  
}
...
representing all the possible evolutionary histories relating the three sequences.  
This transducer is denoted $G_1$ in \secref{EvidenceExpandedModel}.
For clarity, many transitions in this diagram have been removed or collapsed.
}
...
Transducer $E_n$, which is what we mean by the ``constrained-expanded model'', is a stochastically generated subset of $H_n$,
intended to cover as much probability in as few states as possible. $E_n$ is effectively a recognition profile of sequences that most probably appear at node $n$, given the observed descendants of that node.
\figref{viterbi-profile} and \figref{forward-profile} are examples of such transducers recognizing
...
The profile $E_n$ is constructed as follows.
