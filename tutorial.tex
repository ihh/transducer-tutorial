...
\hl{For a maximum profile size of $M$ (chosen by the user), }
...
The complexity drops further to \hl{${\cal O}(cLN)$} if a ``guide alignment'' is provided as a clue to the algorithm.
...
by terminating  stochastic sampling after $\numberofstates{E_*} \geq m$ (and discarding 
the final sampled path, if desired).  
...
We described a hierarchical  approximation technique enabling inference in \hl{ ${\cal O}((cL)^2N)$} time and memory, as opposed to ${\cal O}(L^N)$ for exact, exhaustive inference ($N$ sequences of length $L$). 
Adding additional constraints (in the form of a ``guide alignment'')  bring time and memory complexity down to \hl{ ${\cal O}(cLN)$}, making the algorithm practical for typical alignment problems.  
