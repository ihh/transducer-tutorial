...
this drops to \hl{ ${\cal O}((c\profileSizeLimit)^2 N)$} when
...
indicating that alignment profiles  may also help to avoid the    bias towards deletions
...
where $t_{Z(m)}$ is the time required to compute $Z(m)$. 
...
incoming transition per state), though worst-case size is ${\cal O}(p)$.  
...
TODO: include $\tau$ in other mentions of complexity?  Motivate typical-case complexity and/or 
align env performance?
...
in \hl{ ${\cal O}((c\profileSizeLimit)^2N)$} time and memory, as opposed to ${\cal O}(L^N)$ 
