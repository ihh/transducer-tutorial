...
are collapsed into a single transition, and emission weights are associated with states (\figref{condensed-emission}).
}
...
In this diagram, all four insertion transitions from \figref{fanned-emission} are collapsed into a single transition;
similarly, all four deletion transitions from \figref{fanned-emission} are collapsed into one,
and all sixteen substitution transitions are collapsed to one.
To allow this, the transition weight must factorize into separate emission and transition components, as in \figref{fanned-emission}.
In this example (corresponding to \figref{fanned-emission}), the emission weights are $U(y)$ for insertions, $V(x)$ for deletions, and $Q(x,y)$ for substitutions;
while the transition weights are $f$ for insertions, $h$ for deletions, and $g$ for substitutions.
...
Instead of specific I/O characters ({\tt a/g} for a substitution of A to G, {\tt -/c} for an insertion of C, etc.)
we now have generic labels like ``{\tt x/y}''; the actual characters (and their weights) are encoded by the emission functions.
These labels are still written in blue.
For most Figures in the remainder of this manuscript, we will omit these blue generic I/O labelings,
