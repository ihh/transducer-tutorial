...
\item Algorithms for ancestral sequence reconstruction conditioned on an alignment, including:
 TreeHMM \cite{DialloEtAl2007}, Indelign \cite{KimSinha2007}, and Ortheus \cite{PatenEtAl2008}.
\end{itemize}
...
the four states that can receive an input symbol all have $\upsilon=M$ and $b_1,b_2 \in \{M,D\}$
(i.e. $MMM$, $MMD$, $MDM$ and $MDD$).
Of these,
...
These four states that involve emissions by the generator all have $\rho=I$ and $b_1,b_2 \in \{M,D\}$
(i.e. $IMM$, $IMD$, $IDM$ and $IDD$).
Of these,
