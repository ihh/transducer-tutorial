...
\section{Document structure}  We provide first an informal overview, then a technical section in which we describe the algorithm in detail.  
The informal overview is intended to be easily read, even by someone not familiar with transducer theory. 
...
are possible.  
This can lead to an excess of transitions, complicating the presentation.  
...
In a Mealy-machine-like representation, each possible pair of characters must be given its own transition, cluttering the visualization.}
...
\widepdffig{legend}{Each transducer state must be categorized as one of the above state {\em types} (though a transducer may contain several of each type).  
...
but rather the transitions which are permitted between the {\em types}
...
These are related to \figref{mf-substituter} and \figref{cs-substituter}, but the composition
is reversed.  
...
With this prior, the sum-over-paths sum computes final Felsenstein probability, and 
sampling traceback paths yields samples from the  posterior distribution 
over ancestral sequences.  This ability to sample paths will allow us to constrain
