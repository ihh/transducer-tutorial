...
For example we will compute the likelihood of tree (MF:$t$,LIV:$t$)
under TKF91 model...
\needfig{figure showing the tree (MF:$t$,LIV:$t$)}
...
\figref{mf-generator} is an example of a generator (for the protein sequence MF);
\figref{liv-small} is an example of a recognizer (for the protein sequence LIV).
...
\item \needfigref is the Moore-machine recognizer for sequence CS.
\item \needfigref is a ``null model'' generator that emits a single IID sequence
...
\needfig{cs-labeled Moore-normal form recognizer for protein sequence CS.}
\needfig{null}
...
Discussion of probabilities $a,b,c...$ and parameters $\lambda,\mu,R$
...
\needfig{substituter-substituter2}
...
this is equivalent to doubling the time parameter in \figref{tkf91}...
...
\needfigref is the composition of MF-generator with TKF91...
...
Note also that it computes $G^{(n)}$ from \seclabel{Felsenstein}...
...
where
$b_1$ is the state of the transducer on the CS-branch (\figref{substituter}),
$d_1$ is the state of the exact-matcher for sequence CS (\figref{cs-labeled}),
$b_2$ is the state of the transducer on the MF-branch (\figref{substituter2}),
...
\needfig{root-fork-tkf91mf-tkf91liv-viterbi-traceback}
...
\needfig{fork-tkf91mf-tkf91liv-viterbi-traceback}
...
\needfig{root-fork-tkf91mf-tkf91liv-forward-tracebacks}
...
\needfig{fork-tkf91mf-tkf91liv-forward-tracebacks}
...
on the probability computed by the full transducer in \figref{fork-tkf91liv-tkf91mf}.

\section{Overview of Felsenstein recursion}

We have already seen the individual steps of the transducer version of the Felsenstein recursion...
