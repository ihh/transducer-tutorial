...
Among other things, this restriction ensures that the transducer always goes through a Wait state before inputting a symbol (in a Match or Delete state)...
This is advantageous when choreographing arrangements of several transducers, as in later parts of this tutorial...
The downside is that we sometimes have a few redundant-looking Wait states lingering around...
For most Figures in the remainder of this manuscript, we will leave out the blue ``x/y'' labels on transitions,
as they are implied by the state type of the destination state.
}
...
The notation ($TU$ or $T \cdot U$) reflects the fact that composition is just like matrix multiplication...
...
\widepdffig{tkf91-tkf91}{Transducer $\tkf \cdot \tkf$, the composition of the TKF91 model (\figref{tkf91-labeled}) with itself.
See text for I/O weight functions and explanation of state labels...
}
...
\tallpdffig{mf-tkf91}{Transducer $\generate(MF) \cdot \tkf$, the composition of MF-generator (\figref{moore-mf-generator}) with TKF91 transducer (\figref{tkf91-labeled})}
...
\pdffig{substituter-mf}{Transducer $\substitute(Q) \cdot \recognize(MF)$...}

\pdffig{substituter-cs}{Transducer $\substitute(Q) \cdot \recognize(CS)$...}
...
\pdffig{mf-substituter-cs}{Transducer $\generate(MF) \cdot \substitute(Q) \cdot \recognize(CS)$...}
